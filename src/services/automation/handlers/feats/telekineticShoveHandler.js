...
function isShoveLatchedThisTurn(latch, round) {
    return !!latch && (latch.round ?? 0) >= round;
}

async function refuseShoveAlreadyUsed(action, playerName, campaignName) {
...
function logShoveSaveOutcome({ campaignName, playerName, targetName, saveType, saveDc, pushDistance, auto, success }) {
    const description = success
        ? `${targetName} succeeded on ${saveType} save. No effect.`
        : `${targetName} failed ${saveType} save. Pushed ${pushDistance} feet.`;
...
export async function handle(action, playerStats, campaignName, _mapName) {
    ...
    if (isShoveLatchedThisTurn(latch, round)) {
        return refuseShoveAlreadyUsed(action, playerName, campaignName);
    }
    ...
    logShoveSaveOutcome({ campaignName, playerName, targetName, saveType, saveDc, pushDistance, auto, success });
