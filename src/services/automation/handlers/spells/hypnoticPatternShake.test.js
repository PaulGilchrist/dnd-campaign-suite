...
function makeAction(automation = {}) {
  return { name: 'Shake Out Stupor', automation: { range: '5 ft', ...automation } };
}

const baseCombatContext = {
  creatures: [
    { name: 'Goblin', type: 'monster', conditions: [{ key: 'charmed' }, { key: 'incapacitated' }] },
...
describe('hypnoticPatternShake.handle', () => {
  ...
  describe('target filtering', () => {
    it('should prioritize hypnotic pattern targets over eligible targets', async () => {
    ...
    it('should return eligible targets when no hypnotic pattern targets', async () => {
      const combatNoHypno = {
        creatures: [
          { name: 'Orc', type: 'monster', conditions: [{ key: 'frightened' }] },
          { name: 'TestCaster', gridX: 5, gridY: 10 },
      ...
      getCombatContext.mockResolvedValue(combatNoHypno);
      rangeToFeet.mockReturnValue(5);
      resolveMapPositions.mockResolvedValue(null);
    ...
    it('should default feature name to Shake Out Stupor', async () => {
      getCombatContext.mockResolvedValue(baseCombatContext);
      rangeToFeet.mockReturnValue(5);
      resolveMapPositions.mockResolvedValue(null);
  ...
  describe('range filtering', () => {
    it('should filter targets by range when positions are available', async () => {
      ...
      // Ally is at distance 10 > 5, so it should be filtered out
      expect(result.type).toBe('popup');
      expect(result.payload.description).toContain('No eligible targets');
    });

    it('should use range from automation config', async () => {
      getCombatContext.mockResolvedValue(baseCombatContext);
      rangeToFeet.mockReturnValue(30);
      resolveMapPositions.mockResolvedValue({
        attackerPos: { gridX: 5, gridY: 10 },
        mapData: { players: [{ name: 'TestCaster', gridX: 5, gridY: 10 }] },
      });
      getDistanceFeet.mockReturnValue(10);
      getRuntimeValue.mockReturnValue([]);

      const result = await handle({ automation: { range: '30 ft' } }, makePlayerStats(), campaignName, 'test-map');
  ...
  describe('player type targets', () => {
    it('should check runtime conditions for player type creatures', async () => {
      ...
      getRuntimeValue.mockImplementation((name, prop) => {
        if (prop === 'activeConditions' && name === 'Ally') return ['charmed'];
        return [];
      });
...
describe('hypnoticPatternShake.handleConfirm', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should remove charmed, incapacitated, speed_zero for player type', async () => {
  ...
  it('should remove charmed and incapacitated from creature conditions', async () => {
    getCombatContext.mockResolvedValue({
      creatures: [{ name: 'Goblin', type: 'monster', conditions: [{ key: 'charmed' }, { key: 'incapacitated' }, { key: 'frightened' }] }],
    });
  ...
  it('should log ability_use entry', async () => {
    getCombatContext.mockResolvedValue({
      creatures: [{ name: 'Goblin', type: 'monster', conditions: [{ key: 'charmed' }] }],
    });
  ...
  it('should return popup with success message', async () => {
    ...
    expect(result.payload.description).toContain('no longer affected by Hypnotic Pattern');
  });
});
