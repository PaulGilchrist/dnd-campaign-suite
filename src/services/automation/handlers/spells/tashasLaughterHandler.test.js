...
describe('tashasLaughterHandler.processTashasLaughterRepeatSave', () => {
  ...
  describe('successful repeat save', () => {
    it('should remove Prone and Incapacitated conditions', async () => {
    ...
    it('should clear tracking', async () => {
      ...
      createSaveListener.mockReturnValue({
        promptId: 'laughter-repeat-track',
        promise: Promise.resolve({ success: true }),
      });

      await processTashasLaughterRepeatSave('TestCaster', 'Goblin', 15, campaignName);

      expect(setRuntimeValue).toHaveBeenCalledWith('TestCaster', '_tashas_laughter_Goblin', null, campaignName);
    });
    ...
    it('should clear damage trigger flag', async () => {
      ...
      createSaveListener.mockReturnValue({
        promptId: 'laughter-repeat-damage',
        promise: Promise.resolve({ success: true }),
      });
    ...
    it('should clean up target effect', async () => {
      getRuntimeValue.mockImplementation((caster, key, prop) => {
        if (key === '_tashas_laughter_Goblin') return true;
        if (prop === 'targetEffects') return [{ target: 'Goblin', effect: 'tashas_laughter_repeat_save', source: 'TestCaster' }];
        return true;
      });
      createSaveListener.mockReturnValue({
    ...
    it('should call postLogEntry for condition removal', async () => {
      getRuntimeValue.mockImplementation((target, prop) => {
        if (prop === 'activeConditions') return ['Incapacitated', 'Prone'];
        return true;
      });
      createSaveListener.mockReturnValue({
        promptId: 'laughter-repeat-log',
        promise: Promise.resolve({ success: true }),
      });
  ...
  describe('failed repeat save', () => {
    ...
    it('should call addEntry with save_result on failed repeat save', async () => {
      ...
      createSaveListener.mockReturnValue({
        promptId: 'laughter-repeat-fail-entry',
        promise: Promise.resolve({ success: false }),
      });
