...
describe('reactionDamageHandler', () => {
    ...
    describe('save result handling', () => {
        it('applies damage on save failure', async () => {
            ...
            expect(applyDamageToTarget).toHaveBeenCalledWith(cs, 'Enemy', 5, ['Necrotic'], { campaignName: 'test-campaign', characters: [], ignoreResistance: false, attackerName: 'TestHero' });
        });
        ...
        it('logs hp_change via applyDamageToTarget after the damage roll (Hand of Harm CLA-158)', async () => {
            ...
            expect(applyDamageToTarget).toHaveBeenCalledWith(cs, 'Animated Rug of Smothering 1', 5, ['Necrotic'], { campaignName: 'test-campaign', characters: [], ignoreResistance: false, attackerName: 'TestHero' });
        });
    ...
    describe('CLA-361 Thought Shield persistence + once-per-hit latch', () => {
        ...
        it('persists the reflect through applyDamageToTarget (ignoreResistance, warlock attributed)', async () => {
            ...
            expect(applyDamageToTarget).toHaveBeenCalledWith(expect.objectContaining({ round: 3 }), 'Githzerai Psion 1', 14, ['Psychic'], { campaignName: 'test-campaign', characters: expect.any(Array), ignoreResistance: true, attackerName: 'TestHero' });
            expect(result.type).toBe('popup');
