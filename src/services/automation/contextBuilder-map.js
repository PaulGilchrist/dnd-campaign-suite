...
export function buildAttackContext(attack, playerStats, campaignName, mapName, conditionAttackMode, featRangeEffects, opts) {
    ...
    return Promise.all([
        ...
        return getCombatContext(campaignName).then(async cs => {
            const targetPos = resolveTargetPosition(mapData, cs, playerStats.name, attackerPlayer);

            if (base.forcedMode === undefined) {
                const { mapAdv, mapDis } = await accumulateAuraCounts(base, playerStats, campaignName, mapData, targetPos);
                base._mapAdv = mapAdv;
                base._mapDis = mapDis;
            }
            ...
            if (isRanged && !base.isAutoMiss) {
                applyMeleeProximity(base, attackerPlayer, mapData, npcs, feats);
            }
...
function hasProtectionEffectAgainst(targetName) {
    const storedEffects = getRuntimeValue('campaign', 'targetEffects') || [];
    return storedEffects.some(te => te.effect === 'protection' && te.target === targetName);
}

// When the target has no map position, aura checks fall back to skipRangeCheck mode.
async function accumulateAuraCounts(base, playerStats, campaignName, mapData, targetPos) {
    let mapAdv = 0;
    let mapDis = 0;
    const auraOpts = targetPos
        ? { targetPos, attackerName: playerStats.name, campaignName, mapData }
        : { attackerName: playerStats.name, campaignName, mapData, skipRangeCheck: true };
    const wolfResult = await getWolfAdvantageAgainst(auraOpts);
    if (wolfResult.advantage) {
        mapAdv++;
    }
    const duplicityResult = await getDuplicityAdvantageAgainst(auraOpts);
    if (duplicityResult.advantage) {
        mapAdv++;
    }
    const lionOpts = targetPos
        ? { attackerName: playerStats.name, campaignName, mapData }
        : { attackerName: playerStats.name, campaignName, mapData, skipRangeCheck: true };
    const lionResult = await getLionDisadvantageAgainst(lionOpts);
    if (lionResult.disadvantage) {
        mapDis++;
    }
    if (base.targetName && hasProtectionEffectAgainst(base.targetName)) {
        mapDis++;
    }
    const coronaOpts = { targetName: base.targetName, campaignName, mapData, damageType: base.damageType };
    if (!targetPos) {
