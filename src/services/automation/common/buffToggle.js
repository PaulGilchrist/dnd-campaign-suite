     ...
     const wasActive = activeBuffs.some(b => b.name === actionName);
      ...
      const newBuffs = wasActive
           ? activeBuffs.filter(b => b.name !== actionName)
            : [...activeBuffs, { name: actionName, effect: auto.effect, duration: auto.duration, enemiesDisadvantageSaves: auto.enemies_disadvantage_saves || [], distance: auto.distance || '', extendedDistance: auto.extendedDistance || '', sourceCharacter: playerName, blocksSpellcasting: auto.blocksSpellcasting || false, flySpeed: auto.flySpeed || null, hover: auto.hover || false }];
