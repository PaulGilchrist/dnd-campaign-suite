...
describe('helpers.js — applyPowerWordKillToTarget', () => {
  ...
  it('kills target when HP <= 100', async () => {
    ...
    expect(applyDamageToTarget).toHaveBeenCalledWith(expect.anything(), 'Goblin', 80, ['Psychic'], { campaignName: 'test-campaign', characters: [], ignoreResistance: false, attackerName: 'TestWizard' });
  ...
  it('deals 12d12 Psychic damage when HP > 100', async () => {
    ...
    expect(applyDamageToTarget).toHaveBeenCalledWith(expect.anything(), 'Dragon', expect.any(Number), ['Psychic'], { campaignName: 'test-campaign', characters: [], ignoreResistance: false, attackerName: 'TestWizard' });
  });
