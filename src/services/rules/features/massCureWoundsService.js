...
export async function triggerMassCureWounds(spell, metaCtx, playerStats, campaignName, _mapName) {
    ...
    for (const target of targets) {
        ...
        if (storedMaxHp == null) {
            console.error(`[massCureWoundsService] maxHp missing for target ${target.name || 'unknown'}`, { stack: new Error().stack });
        }
        ...
        if (storedHp != null && storedHp !== '') {
            ...
            console.error(`[massCureWoundsService] storedHp not tracked for target ${targetName}, defaulting to maxHp`, { stack: new Error().stack });
            currentHp = maxHp;
        }
