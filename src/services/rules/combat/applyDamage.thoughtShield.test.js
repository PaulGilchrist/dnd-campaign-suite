...
describe('Thought Shield — manual reaction handler', () => {
  ...
  describe('successful reflection — persisted via applyDamageToTarget', () => {
    ...
    it('routes the reflect through applyDamageToTarget with ignoreResistance and logs ability_use', async () => {
      ...
      expect(applyDamageToTarget).toHaveBeenCalledWith(expect.objectContaining({ round: 1 }), 'Goblin', 5, ['Psychic'], { campaignName: 'TestCampaign', characters: expect.any(Array), ignoreResistance: true, attackerName: 'Warlock' });
    ...
    it('uses rawDamage as fallback when actualDamage is missing', async () => {
      ...
      expect(applyDamageToTarget).toHaveBeenCalledWith(expect.anything(), 'Goblin', 8, ['Psychic'], { campaignName: 'TestCampaign', characters: expect.any(Array), ignoreResistance: true, attackerName: 'Warlock' });
