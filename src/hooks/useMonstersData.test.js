...
import { useMonstersData } from './useMonstersData.js';
...
describe('useMonstersData', () => {
  ...
  it('should start in loading state', async () => {
    ...
    const { result } = renderHook(() => freshHook('5e'));
  ...
  it('should load monsters from 5e path', async () => {
  ...
  it('should load monsters from 2024 path', async () => {
    const mockMonsters = [{ name: 'Goblin 2024', challenge_rating: '1/4' }];
    global.fetch = vi.fn(() =>
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve(mockMonsters),
    ...
    const { result } = renderHook(() => freshHook('2024'));
  ...
  it('should default to 5e when no rulesVersion provided', async () => {
  ...
  it('should not update state after unmount', async () => {
    ...
    const { unmount } = renderHook(() => freshHook('5e'));

    unmount();

    // After unmount, the cancelled flag prevents state updates
