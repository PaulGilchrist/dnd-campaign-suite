...
describe('SP-099 — Resistance consumer re-reduces once after per-turn re-arm', () => {
    ...
    function attack(attackerName) {
        ...
        return fn({ name: 'Mace', formula: '1d6+2', total: 8, rolls: [6], modifier: 2, context: { targetName: 'Goblin', damageType: 'bludgeoning', attackerName, } });
    }
    ...
    it('non-chosen damage type is never reduced even when re-armed', async () => {
        state.used = false;
        const fn = createLogDamageAndShow(deps);
        await fn({ name: 'Dagger', formula: '1d4+2', total: 6, rolls: [4], modifier: 2, context: { targetName: 'Goblin', damageType: 'piercing', attackerName: 'Thug 2', } });
