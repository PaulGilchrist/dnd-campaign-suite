    ...
    if (context?.overchannelActive) {
        if (context?.overchannelUseCount > 1) {
            ...
            if (necroticResult) {
                ...
                const casterApplyResult = await applyDamageToTarget(casterCombatSummary, characterName, necroticResult.total, ['Necrotic'], { campaignName, characters: characters, ignoreResistance: true, attackerName: characterName });
                logEntry({
                    type: 'roll',
                    characterName,
