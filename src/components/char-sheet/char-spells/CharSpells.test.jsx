...
// Mock useSpellMetamagicFlow - use module-level mocks for dynamic control
...
// Mock useSpellUpcastFlow - use module-level mocks for dynamic control
const mockGateUpcast = vi.fn(() => false);
const mockGetCantripAutoLevel = vi.fn(() => null);
...
describe('CharSpells', () => {
 ...
 it('should show spell detail popup when spell name is clicked', () => {
  ...
  // ===== cannotAct / exhaustionPenalty / conditionAttackMode tests =====
  ...
  // ===== Spells with no components / no casting_time / no duration =====
  ...
  // ===== Casting time abbreviations for reaction and bonus action =====
  ...
  // ===== spells_known fallback for prepared_spells display =====
  ...
  // ===== Cantrip damage level selection =====
  ...
  // ===== Popup with string HTML content =====
  ...
  // ===== Dice popup with waitingForPlayerSave =====
  ...
  // ===== Spell with no damage_type fallback =====
  ...
  // ===== Spell with empty damage_at_slot_level / damage_at_character_level =====
  ...
  // ===== gateUpcast returning true (blocks damage roll) =====

  it('should not call executeDamageRoll when gateUpcast returns true', () => {
    mockGateUpcast.mockReturnValue(true);

    const statsWithDamageSpell = {
      ...mockPlayerStats,
      spellAbilities: {
        ...mockPlayerStats.spellAbilities,
        spells: [
    ...
    render(
      <CharSpells
        playerStats={statsWithDamageSpell}
        handleTogglePreparedSpells={mockHandleTogglePreparedSpells}
      />
    );

    const effectCell = screen.getByText('8d6 Fire');
    fireEvent.click(effectCell);

    // gateUpcast returned true, so rollDamage should NOT be called
    expect(mockRollDamage).not.toHaveBeenCalled();
  });

  // ===== getCantripAutoLevel returning a level (cantrip upcast) =====
  ...
  // ===== Spell cast via spell detail popup triggers gateMetamagic =====
  ...
  // ===== Spell with no damage_type in damage object =====

  it('should handle spell damage where damage_type is missing', () => {
    const statsWithMissingType = {
      ...mockPlayerStats,
      spellAbilities: {
        ...mockPlayerStats.spellAbilities,
        spells: [
    ...
    render(
      <CharSpells
        playerStats={statsWithMissingType}
        handleTogglePreparedSpells={mockHandleTogglePreparedSpells}
      />
    );
  ...
  // ===== Spell with no components array (undefined) =====
  ...
  // ===== Sort by level with same-level spells (tests localeCompare branch) =====
  ...
  // ===== Spell attack button (non-sorcerer path) =====
  ...
  // ===== cannotAct prevents spell attack click =====
  ...
  // ===== conditionAttackMode passed to rollAttack =====
  ...
  // ===== Sorcerer spell attack triggers metamagic popup =====
  ...
  // ===== Normal (non-sorcerer) spell attack calls rollAttack =====
  ...
  // ===== Spell with reaction casting time =====
  ...
  // ===== Spell with bonus action casting time =====
  ...
  // ===== Spell with concentration and ritual combined =====
  ...
  // ===== Duration with concentration and minutes abbreviation =====
  ...
  it('should handle spell with empty range', () => {
  ...
  // ===== Multiple spells with different levels for sorting =====
  ...
  // ===== Spell with only damage_at_character_level (no slot level) =====
  ...
  // ===== Spell with damage but no damage_type shows empty type =====
  ...
  // ===== Spell with dc_success other than 'half' shows 'negates' =====
  ...
  // ===== Spell with no damage shows Utility =====
