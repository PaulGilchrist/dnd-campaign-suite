...
describe('CharActions custom event listeners', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
  ...
  describe('soulstitch-modal-show event', () => {
    it('sets soulstitchSpellsModal when the custom event fires', async () => {
      const mockSetSoulstitchSpellsModal = vi.fn();
      vi.mocked(useCharActionModals).mockReturnValue({
        ...useCharActionModals(),
        soulstitchSpellsModal: null,
        setSoulstitchSpellsModal: mockSetSoulstitchSpellsModal,
      });

      await act(async () => { render(<CharActions playerStats={createStats()} />); });

      await act(async () => {
        window.dispatchEvent(new CustomEvent('soulstitch-modal-show', { detail: { spells: ['Magic Missile'] } }));
      });

      expect(mockSetSoulstitchSpellsModal).toHaveBeenCalledWith({ spells: ['Magic Missile'] });
    });

    it('removes the event listener on unmount', async () => {
      const removeEventListenerSpy = vi.spyOn(window, 'removeEventListener');
      const { unmount } = await act(async () => { return render(<CharActions playerStats={createStats()} />); });
      unmount();
      expect(removeEventListenerSpy).toHaveBeenCalledWith('soulstitch-modal-show', expect.any(Function));
      removeEventListenerSpy.mockRestore();
    });
  });

  describe('sweeping-attack-modal-show event', () => {
    it('sets sweepingAttackTargetModal when the custom event fires', async () => {
      const mockSetSweepingAttackTargetModal = vi.fn();
      vi.mocked(useCharActionModals).mockReturnValue({
        sweepingAttackTargetModal: null,
      ...
      await act(async () => {
        window.dispatchEvent(new CustomEvent('sweeping-attack-modal-show', { detail: { selectedTarget: 'Goblin 1' } }));
      });

      expect(mockSetSweepingAttackTargetModal).toHaveBeenCalledWith({ selectedTarget: 'Goblin 1' });
    });
  });

  describe('bait-and-switch-modal-show event', () => {
    it('sets baitAndSwitchChoiceModal when the custom event fires', async () => {
      const mockSetBaitAndSwitchChoiceModal = vi.fn();
      vi.mocked(useCharActionModals).mockReturnValue({
        baitAndSwitchChoiceModal: null,
        setBaitAndSwitchChoiceModal: mockSetBaitAndSwitchChoiceModal,
      });

      await act(async () => { render(<CharActions playerStats={createStats()} />); });

      await act(async () => {
        window.dispatchEvent(new CustomEvent('bait-and-switch-modal-show', { detail: { selectedTarget: 'Orc 1' } }));
      });

      expect(mockSetBaitAndSwitchChoiceModal).toHaveBeenCalledWith({ selectedTarget: 'Orc 1' });
    });
  });

  describe('commander-strike-modal-show event', () => {
    it('sets commanderStrikeChoiceModal when the custom event fires', async () => {
      const mockSetCommanderStrikeChoiceModal = vi.fn();
      vi.mocked(useCharActionModals).mockReturnValue({
        commanderStrikeChoiceModal: null,
        setCommanderStrikeChoiceModal: mockSetCommanderStrikeChoiceModal,
      });

      await act(async () => { render(<CharActions playerStats={createStats()} />); });

      await act(async () => {
        window.dispatchEvent(new CustomEvent('commander-strike-modal-show', { detail: { selectedTarget: 'Giant' } }));
      });

      expect(mockSetCommanderStrikeChoiceModal).toHaveBeenCalledWith({ selectedTarget: 'Giant' });
    });
  });
});
