...
describe('TeleportModal', () => {
  ...
  // ── Close behavior across all modal variants ──

  describe('close behavior across all modal variants', () => {
    it('closes on overlay click for standard modal', () => {
      const onClose = vi.fn();
      render(<TeleportModal action={makeAction()} {...makeProps({ onClose })} />);
      fireEvent.click(getOverlay());
      expect(onClose).toHaveBeenCalledTimes(1);
    });
    ...
    it('closes on overlay click for elemental stride modal', () => {
      const onClose = vi.fn();
      render(<TeleportModal action={makeAction()} {...makeProps({ triggeredByElementalStride: true, onClose })} />);
      fireEvent.click(getOverlay());
      expect(onClose).toHaveBeenCalledTimes(1);
    });
