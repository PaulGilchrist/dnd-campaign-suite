...
describe('CharacterCreationWizard - Editing mode', () => {
  ...
  it('shows Save Changes on the last step when editing', () => {
    useWizardNavigation.mockImplementation(() => ({
      currentStep: 12,
      isNextDisabled: false,
      navigateNext: mockNavigateNext,
    ...
    render(<CharacterCreationWizard {...defaultProps} isEditing={true} />);
    expect(screen.getByText('Save Changes')).toHaveClass('btn-success');
  });

  it('shows Next button (not Save Changes) when editing on non-last step', () => {
    useWizardNavigation.mockImplementation(() => ({
      currentStep: 5,
      isNextDisabled: false,
      navigateNext: mockNavigateNext,
