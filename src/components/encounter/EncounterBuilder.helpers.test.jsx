...
// We need to extract the helper functions for testing.
// Since they're not exported, we test them indirectly through the component
// or by re-implementing the pure logic here for direct testing.

describe('EncounterBuilder helper functions', () => {
  ...
  describe('updateQty', () => {
    ...
    it('removes monster when qty reaches 0', () => {
      const selected = [{ index: 'goblin', name: 'Goblin', xp: 50, qty: 1 }];
      const result = updateQty(selected, 'goblin', 1);
      expect(result).toEqual([{ index: 'goblin', name: 'Goblin', xp: 50, qty: 2 }]);
    });
  ...
  describe('filterMonsters', () => {
    // Replicates the actual helper from EncounterBuilder.jsx
    // which uses calculateXPThreshold from encounterGenerator.js
    const xpThresholds = [
    ...
    it('filters by search query matching name', () => {
      // With 3 players at level 1, difficulty 1 (Medium): threshold = 3*50=150, maxXP=225, minXP=150*0.15=22.5
    ...
    it('filters by search query matching type', () => {
      // Level 15 medium threshold = 2800 per player, 3 players = 8400, maxXP=12600
    ...
    it('excludes monsters below min XP threshold', () => {
      // With threshold=0, minXP=0, so all pass the min check
      // Need levels high enough that maxXP covers all monsters
  ...
  describe('formatEncounterName', () => {
    ...
    it('converts kebab-case to title case', () => {
      expect(formatEncounterName('goblin-cave')).toBe('Goblin Cave');
    });
    ...
    it('handles single word', () => {
      expect(formatEncounterName('ambush')).toBe('Ambush');
    });
    ...
    it('handles multiple hyphens', () => {
      expect(formatEncounterName('multi-word-encounter-name')).toBe('Multi Word Encounter Name');
    });
  });
});
