// MA-0163 regression: Ancient Black Dragon Frightful Presence (legendary_actions[2])
// previously authored only name/description — the header uses:3 gate (MA-0161) made
// the "Expend Legendary" chip live, but the row still spent a use into the
// resolveLegendaryRowMechanic console.error dead-end (no save_dc → no prompt, no
// condition, no te). Data fix mirrors the MA-0147 Adult White FP exact shape
// (objects, NOT booleans): save_effect (frightened) + dc_success:"none" +
// success_immunity + repeat_save. save_type follows the SPELL's dc_type (spells.json
// Fear dc_type WIS → Wisdom), matching Adult Black/Dracolich precedents; save_dc =
// authored Spellcasting save_dc 21.
...
const ABD = monstersData.find(m => m.name === 'Ancient Black Dragon');
const FP = ABD.legendary_actions.find(a => a.name === 'Frightful Presence');
const WHITE_FP = monstersData.find(m => m.name === 'Adult White Dragon').legendary_actions.find(a => a.name === 'Frightful Presence');

const CREATURES = [
  { name: 'Ancient Black Dragon 1', type: 'npc', targetName: 'TestPC', currentHp: 367, maxHp: 367, ac: 22, conditions: [] },
  { name: 'TestPC', type: 'player', currentHp: 143, maxHp: 143, conditions: [] },
];

function fpSaveChip() {
  return Array.from(document.querySelectorAll('.mc-dice-link-save-clickable')).find(el => el.textContent.includes('DC 21 Wisdom')) || null;
}

describe('MA-0163 monsters.json data: Ancient Black Dragon FP row full-service shape', () => {
  it('FP row: DC 21 + Wisdom save (spell dc_type follows Fear) + frightened save_effect + dc_success none', () => {
  ...
  it('FP row engages the MA-0048 service: repeat_save + success_immunity object shape (MA-0147 exact shape, objects not booleans)', () => {
    ...
    expect(Object.keys(FP)).toEqual(Object.keys(WHITE_FP));
  ...
  it('immunity te on target refuses the FP row click (MA-0030/MA-0048 gate)', () => {
    ...
    const active = gazeImmunityActive({
      action: FP, target, monsterName: 'Ancient Black Dragon 1',
      targetEffects: [{ target: 'TestPC', effect: 'frightful_presence_immunity', source: 'Ancient Black Dragon 1' }],
    });
    expect(active).toMatchObject({ effect: 'frightful_presence_immunity', durationMinutes: 1440 });
    const none = gazeImmunityActive({ action: FP, target, monsterName: 'Ancient Black Dragon 1', targetEffects: [] });
    expect(none).toBeNull();
  });
});

describe('MA-0163 MonsterCardModal FP save roll context', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    Object.keys(runtime.store).forEach(k => delete runtime.store[k]);
    ctx.value = { round: 1, activeCreatureName: 'TestPC', creatures: CREATURES };
    runtime.store['Ancient Black Dragon 1.monsterLegendaryUses'] = { max: 3, used: 0 };
  });

  it('legendary FP click opens WISDOM save at DC 21 with frightened conditions, dcSuccess none, repeatSave + successImmunity forwarded', async () => {
    const m = makeMonster({ ...ABD, name: 'Ancient Black Dragon' });
    render(<MonsterCardModal {...makeProps(m, { creatureName: 'Ancient Black Dragon 1', creatures: CREATURES })} />);
    ...
    expect(call[2]).toMatchObject({
      ...
      attackerName: 'Ancient Black Dragon 1',
