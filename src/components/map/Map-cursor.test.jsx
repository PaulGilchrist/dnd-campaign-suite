...
describe('Map cursor styles', () => {
    it('should show grab cursor when tool is none and not panning', async () => {
    ...
    it('should show crosshair cursor when rulerMode is true', async () => {
        ...
        const svg = container.querySelector('svg.grid-svg');
        expect(svg.getAttribute('style')).toContain('cursor: crosshair');
    });

    it('should show default cursor when tool is paint', async () => {
        const { container } = render(
            <Map campaignName="test" characters={[]} isLocalhost={true} mapName="test-map" onBack={vi.fn()} />
        );

        // Tool state is internal to the component, we can't easily change it via mocks
        // The cursor logic is: paint/erase → default, select → crosshair/grabbing, room → crosshair, none → grab
